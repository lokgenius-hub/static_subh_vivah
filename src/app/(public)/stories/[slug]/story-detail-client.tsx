...
import { getSuccessStoryBySlug, getSuccessStories } from "@/lib/client-actions";
...
export default function StoryDetailClient() {
  ...
  const [otherStories, setOtherStories] = useState<SuccessStory[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!slug) return;
    (async () => {
      const s = await getSuccessStoryBySlug(slug);
      if (!s) { setLoading(false); return; }
      setStory(s);
      const all = await getSuccessStories();
      setOtherStories(all.filter(x => x.slug !== slug).slice(0, 3));
      setLoading(false);
    })();
  }, [slug]);
  ...
  if (!story) {
    return <div className="min-h-screen flex flex-col items-center justify-center"><h1 className="text-2xl font-bold mb-2">Story Not Found</h1></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[var(--color-cream)]">
      ...
      <div className="relative h-[50vh] lg:h-[60vh]">
        {story.cover_image ? (
          <Image src={story.cover_image} alt={story.couple_name} fill className="object-cover" priority />
        ) : (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 lg:p-10">
          <div className="mx-auto max-w-4xl">
            ...
            <h1 className="text-3xl lg:text-5xl font-bold text-white mb-2">{story.couple_name}</h1>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        <article className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 mb-10">
          <div className="prose prose-lg max-w-none prose-headings:text-[var(--color-charcoal)] prose-a:text-[var(--color-primary)]" dangerouslySetInnerHTML={{ __html: story.story }} />
        </article>
        ...
        <div className="bg-gradient-to-r from-[var(--color-primary)]/5 to-[var(--color-accent)]/5 rounded-2xl p-8 text-center border border-[var(--color-primary)]/10">
          ...
          <h3 className="text-2xl font-bold text-[var(--color-charcoal)] mb-2">Plan Your Dream Wedding</h3>
          <p className="text-gray-500 mb-6">Let VivahSthal help you create your own beautiful love story in {story.location}.</p>
          <div className="flex gap-3 justify-center">
            <Link href="/venues"><Button size="lg">Find Venues <ArrowRight className="h-4 w-4" /></Button></Link>
